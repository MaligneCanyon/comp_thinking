...
// use the online data file (loaded by opening mail_count.html) or uncomment the following line:
...
function mailCount(emailData) {
  ...
  console.log(`Date Range: ${formatDate(minDate)} - ${formatDate(maxDate)}`);
}

function formatDate(ms) {
  let date = new Date(ms);
  let day = date.toLocaleDateString(undefined, {weekday: 'short'});
  let mth = date.toLocaleDateString(undefined, {month: 'short'});

  return [day, mth, date.getDate(), date.getFullYear()].join(' ');
}
